...
def export_person_to_excel(user) -> tuple[str, str]:
    ...
    headers = [
        ...
        '户籍迁入日期', '是否人户分离', '实际居住地', '是否已迁出', '迁出日期', '迁往地',
        '是否已死亡', '死亡日期', '民族', '政治面貌', '婚姻状况', '文化程度',
        '工作学习情况', '健康状况', '备注'
    ]

    comments = [
        '必填，真实姓名', '可选，18位身份证号（无证人员可留空）', '男/女（支持：男、男性、M、1；女、女性、F、0）', '格式：YYYYMMDD', '必填，多个电话用;分隔',
        '必填，系统内小区名称', '可选，如1单元101室', '自动关联', '常住人口/流动人口', '1=是，0=否',
        '多个类别用,分隔，如独居老人,低保户', '本社区内户籍小区', '外地户籍填写完整地址', '如001、A001（同一户内相同）',
        '格式：YYYYMMDD', '1=是，0=否', '人户分离时填写实际居住地址', '1=是，0=否', '格式：YYYYMMDD', '迁往省市区',
...
def import_person_from_excel(file, user) -> tuple[bool, str]:
    ...
    try:
        ...
        required_columns = ['姓名', '联系电话', '现住小区/建筑']
        ...
        for idx, row in df.iterrows():
            ...
            if raw_gender in ['男', '男性', 'M', '1']:
                gender = '男'
            elif raw_gender in ['女', '女性', 'F', '0']:
                gender = '女'
            else:
                gender = None
            ...
            record = {
                ...
                'household_entry_date': str(row.get('户籍迁入日期', '')).strip(),
                'is_separated': str_to_bool(row.get('是否人户分离', '')),
                'current_residence': str(row.get('实际居住地', '')).strip(),
                'is_migrated_out': str_to_bool(row.get('是否已迁出', '')),
                'household_exit_date': str(row.get('迁出日期', '')).strip(),
                'migration_destination': str(row.get('迁往地', '')).strip(),
                'is_deceased': str_to_bool(row.get('是否已死亡', '')),
                'death_date': str(row.get('死亡日期', '')).strip(),
                'nationality': str(row.get('民族', '')).strip(),
                'political_status': str(row.get('政治面貌', '')).strip(),
                'marital_status': str(row.get('婚姻状况', '')).strip(),
                'education': str(row.get('文化程度', '')).strip(),
                'work_study': str(row.get('工作学习情况', '')).strip(),
                'health': str(row.get('健康状况', '')).strip(),
            }

            try:
                # 关键修复：使用 create_person 单条插入，确保每次都 commit
                create_person(**record)
                success_count += 1
            except Exception as row_e:
                error_msg = str(row_e)
                if "UNIQUE constraint failed: person.id_card" in error_msg:
                    fail_reasons.append(f"第 {idx+2} 行：身份证号 {id_card or '(空)'} 已存在（重复人员，自动跳过）")
                else:
                    fail_reasons.append(f"第 {idx+2} 行：{error_msg[:100]}")
