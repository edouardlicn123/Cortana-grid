...
def get_user_by_username(username: str) -> Optional[Dict]:
    ...
    Returns:
        Optional[Dict]: 用户完整字段字典，不存在或已软删除返回 None
    """
...
def update_user_password(user_id: int, new_password: str) -> bool:
    """
    更新用户密码（接受明文密码，内部自动哈希，并清除强制改密标记）
...
def reset_user_password(user_id: int, new_password: str = 'a12345678') -> bool:
    """
    重置用户密码为默认值，并标记为下次登录必须修改

    Args:
        user_id: 用户 ID
        new_password: 重置后的明文密码（默认 'a12345678'）
...
def get_all_users() -> List[Dict]:
    """
    获取所有用户列表（用于系统设置 → 用户管理页面）

    Returns:
        List[Dict]: 用户列表，每项包含 roles 字段（列表形式）
    """
    query = """
        SELECT 
            u.id, 
    ...
    try:
        ...
        logger.info(f"成功加载用户列表：共 {len(users)} 名用户")
        return users
